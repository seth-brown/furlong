...
export const manhattan = (v1:Vector, v2:Vector) => {
  return v1.map((el, i) => { 
    return Math.abs(el - v2[i]) }).reduce(uf.psum)
}

export const braycurtis = (v1:Vector, v2:Vector) => {
  const n = v1.map((el, i) => { return Math.abs(el - v2[i]) })
    .reduce(uf.psum)
...
export const chebyshev = (v1:Vector, v2:Vector) => {
  return v1.map((el, i) => { return Math.abs(el - v2[i]) })
    .reduce(uf.pmax)
}
